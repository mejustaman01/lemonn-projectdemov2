...
const PatientActivities: React.FC = () => {
  ...
  const [activityProgress, setActivityProgress] = useState<Record<number, number>>({
    1: 0, // Animal Recognition
    2: 0, // Emotion Matching
    3: 0  // Following Directions
  });
  ...
  const updateProgress = (activityId: number, isCorrect: boolean) => {
    const activity = activities.find(a => a.id === activityId);
    if (!activity) return;

    const stepProgress = 100 / activity.steps.length;
    const currentProgress = activityProgress[activityId] || 0;

    if (isCorrect) {
      const newProgress = Math.min(100, currentProgress + stepProgress);
      setActivityProgress(prev => ({
        ...prev,
        [activityId]: Math.round(newProgress)
      }));
    }
  };
  ...
  const handleAnswerClick = (answer: string) => {
    ...
    if (isCorrect && currentActivity) {
      updateProgress(currentActivity, true);
    }
                  ...
                  <div className="p-6">
                    ...
                    <div className="mt-4">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium">{activityProgress[activity.id]}%</span>
                      </div>
                      <div className="mt-1 w-full bg-gray-200 rounded-full h-2">
                        <div
                          className="bg-green-600 h-2 rounded-full transition-all duration-300"
                          style={{ width: `${activityProgress[activity.id]}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="mt-6">
                      ...
                      >
                        {activityProgress[activity.id] === 100 ? 'Practice Again' : 'Start Activity'}
                      </Button>
                    </div>
                  </div>
                </div>
